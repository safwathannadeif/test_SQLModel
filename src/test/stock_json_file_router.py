...
@test_router_json.get('/test_stock/{test_stock_symb}', status_code=200, tags=["stock_fromjson"])
async def get_stock(test_stock_symb: str) -> StockBase:
    print(f"input stock_symbol {test_stock_symb}")
    stock_t = [t_stock for t_stock in await get_test_stocks() if t_stock['symbol'] == test_stock_symb]
    if len(stock_t) == 0:
        raise HTTPException(
